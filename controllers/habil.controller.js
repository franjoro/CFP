...
habil.form = async (req, res) => {
    try {
        ...
        const { DUI, nombre, telefono, email, genero } = global_json1;
        const { [0]: { cantidad } } = await pool.query("SELECT COUNT(*) AS cantidad FROM tb_participantes WHERE DUI = ?", [DUI]);
        if (!cantidad) await pool.query("INSERT INTO tb_participante(DUI, Nombre, Telefono , Email, Genero) VALUES(? ,? , ? ,? ,? )", [DUI, nombre, telefono, email, genero]);

...
readHabil.readDetWhitDUI = async(req,res) =>{
    ...
    try {
        ...
        FROM tb_habil_solicitudes WHERE  REPLACE(JSON_EXTRACT(json1, '$.dui'), '"','' ) = ?;
        `;
        const dataJson1 = await pool.query(sqlJson1,[dui]);
        const sqlJson2 = `
        SELECT 
        ...
        FROM tb_habil_solicitudes WHERE  REPLACE(JSON_EXTRACT(json1, '$.dui'), '"','' ) = ?;
        `;
        const dataJson2 = await pool.query(sqlJson2,[dui]);
        const sqlJson3 = `
        SELECT 
        ...
        FROM tb_habil_solicitudes WHERE REPLACE(JSON_EXTRACT(json1, '$.dui'), '"','' ) = ?;
        `;
        const dataJson3 = await pool.query(sqlJson3,[dui]);

...
cursos.cursos = async (req, res) => {
  ...
  try {
    ...
    queries.push(
      pool.query(
        `SELECT CONCAT(tb_cursos.Codigo_curso,' - ', tb_cursos.Nombre) AS Nombre, tb_instructor.Nombre AS instructor , 
        tb_cursos.Date_inicio as fecha_inicio, tb_cursos.Date_fin as fecha_fin, tb_cursos.Horario , tb_cursos.Codigo_curso , 
        (SELECT COUNT(*) FROM union_matricula WHERE id_curso = tb_cursos.Codigo_curso ) AS cantidadAlumnos FROM tb_cursos 
        INNER JOIN tb_instructor ON tb_cursos.id_instructor = tb_instructor.DUI WHERE tb_cursos.id_programa = ? 
        AND tb_cursos.Estado != 0 AND tb_cursos.Estado != 5 ORDER BY tb_cursos.Codigo_curso ASC;`,
        [programa]
      )
    );
    ...
    queries.push(
      pool.query(
        ...
        (SELECT COUNT(DISTINCT id_empresa) FROM union_curso_empresa WHERE id_curso = tb_cursos.Codigo_curso ) AS cantidadEmpresas , 
...
cursos.curso_detalle = async (req, res) => {
  ...
  try {
    ...
    if (tipo === "oferta") {
      typeQuery = `SELECT CONCAT(Nombre,' - ',Horario) AS Nombre , Nombre as nombre_oferta, Codigo_curso  , Date_inicio , Horario, Fechas, Nombre AS CursoName , CostoAlumno AS costo , horas  FROM tb_cursos WHERE Codigo_curso  = ?`;
    }
...
cursos.editOferta = async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE tb_cursos SET Nombre = ?, Fechas = ? , Horario = ? , Date_inicio= ?, horas = ? WHERE Codigo_curso = ?",
      data
    );
    return res.status(200).json({ status: true });
  } catch (error) {

...
programa.loadTable = async (req, res) => {
  const data = await pool.query(
    "SELECT Nombre,ImgPortada,id_programa FROM tb_programa WHERE Estado = 1"
  );
...
programa.addEncargado = async (req, res) => {
  try {
    ...
    let exist = await pool.query("SELECT COUNT(*) AS c FROM union_programa_usuario WHERE id_usuario =?  AND id_programa = ?",data)
    //Si ya existe returnar a la misma pag
    if(exist[0].c) return res.redirect(`/admin/programa/id${data[1]}`); 
    //Ingresar si no existe
    await pool.query("INSERT INTO union_programa_usuario(id_usuario,id_programa) VALUES(?,?)",data);
    //Retornar
...
programa.deleteinstructor = (req,res) =>{
  res.json(req.body)
}
